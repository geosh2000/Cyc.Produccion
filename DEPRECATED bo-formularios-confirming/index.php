...
//List Seguimientos
$query="SELECT * FROM bo_seguimiento WHERE area=1 ORDER BY actividad, seguimiento";
$result=mysql_query($query);
$num=mysql_numrows($result);
$i=0;
while($i<$num){
...
$(function() {
    ...
    function sendRequest(variables,hora,act,fecha,em){
        var urlsend= "/json/formularios/bo_confirming.php?"+variables;
        var xmlhttp;
        var text;
...
$query="SELECT confirming_id, b.actividad, em, date_created, fecha_recepcion FROM bo_confirming a, bo_actividades b WHERE a.actividad=b.bo_act_id AND date_created>='".date('Y-m-d')."' AND user=".$_SESSION['id'];
$result=mysql_query($query);
$num=mysql_numrows($result);
$i=0;
while($i<$num){
...
<table width='100%' class='t2'>
        <tr class='title'>
            <td colspan=100>Formulario Confirming</td>
        </tr>
    </table>
    <br>
