...
//Get Existing Users
$query="SELECT
	x.*, p.PDV, m.Ciudad as Locacion, z.Departamento, y.Puesto, userid, profile, profile_name, q.esquema, if(CURDATE()<=Egreso,1,0) as Active, CONCAT(Usuario,'@pricetravel.com') as correo_price, FindSupPDVDay(q.oficina,CURDATE(),2) as Supervisor
	FROM
		(
		SELECT
  ...
  ON
	...
	LEFT JOIN
			(SELECT
				userid, username, profile, profile_name
				FROM
	...
	ON
		x.Usuario=y.username
	LEFT JOIN
		db_municipios m
	ON
		q.ciudad=m.id
	HAVING
		`id Departamento`!=-1 AND AsDep IN (29,43)
	ORDER BY
			z.Departamento, Nombre";

$shownCols=array(0,2,3,4,5,6,13,37,36,11,12,35,38,30,31,25,29,28,34,33,32,15,16,17,18,19,20,21);
  ...
  <div id="accordion-Create">
    ...
    <div>
      <fieldset id='fieldset_add'>
          ...
          <label for="new_numcol">Num. Colaborador <span style='font-size= 10px; font-style: oblique;'>(opcional)</span></label>
	        <input title='Num Colaborador' type='text' id='new_numcol' name="new_numcol"><br>
					<label for="new_sup">Supervisor <span style='font-size= 10px; font-style: oblique;'>(opcional)</span></label>
	        <input title='Supervisor' type='text' id='new_sup' name="new_sup"><input type='hidden' id='new_sup_id' name="new_sup_id"><br><br>
	        ...
	        <select id="new_ciudad" name="new_ciudad" title="Ciudad" required><option value=''>Selecciona...</option></select><br><br>
					<label for="new_pdv">PDV</label>
	        <select id="new_pdv" name="new_pdv" title="PDV" required><option value=''>Selecciona...</option></select><br><br>
					<label for="new_dep">Departamento</label>
	        <select id="new_dep" name="new_dep" title="Departamento" required><option value=''>Selecciona...</option><option value='29'>PDV</option></select><br><br>
...
echo "<div class='tabcontain' style='position: relative'>";
