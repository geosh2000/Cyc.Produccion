...
$query="SELECT id, fecha, NombreAsesor(id_asesor,2) as Asesor1, NombreAsesor(`id_asesor 2`,2) as Asesor2, "
		...
		."caso, `Last Update` as Capturado FROM `Cambios de Turno` WHERE id_asesor IN ($asesor1, $asesor2) AND tipo IN (1,2) AND MONTH(Fecha) = MONTH('$Fecha1') AND YEAR(Fecha) = YEAR('$Fecha1')";
			//echo "$query";
