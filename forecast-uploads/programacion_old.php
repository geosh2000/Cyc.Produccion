...
if(isset($_POST['submit']) || $_POST['copy']==1){
	...
	if($draft==1){
		...
		$db_order="`jornada start`, Nombre, Departamento, Esquema";
		$db_skill="";
	}
	
	$query="SELECT a.id, `N Corto`, Nombre, Esquema, `jornada start`, `jornada end`, `comida start`, `comida end`, `extra1 start`, `extra1 end`, `extra2 start`, `extra2 end`, getAusentismo(a.id,'$inicio',1) as Ausentismo, `id Departamento` as Departamento FROM Asesores a LEFT JOIN "
			."(SELECT * FROM $db WHERE Fecha='$q_inicio' ".$db_skill.") b "
			." ON a.id=b.asesor WHERE Egreso>'$inicio'"; 
			if($skill==35){
				$query.=" AND `id Departamento` IN (35,43)";
			}else{
				$query.=" AND `id Departamento`=$skill";
			}
...
<table class='t2' style='width:600px; margin:auto'><form action="programacion.php" id='form_consulta' method="post" enctype="multipart/form-data">
	<tr class='title'>
		<th colspan=10>Programacion <?php if(isset($_POST['submit']) || $_POST['copy']==1){echo " ($depart $inicio a $fin)"; if($draft==1){echo " (From draft)";}} ?></th>
	</tr>
	<tr class='title'>
		<td style='width:33%'>Periodo</td>
		<td style='width:33%'>Programa</td>
		<td style='width:20%'>From<br>draft</td>
		<td rowspan=3 class='total'><input type="submit" id='submit' value="Consultar" name="submit"></td>
	</tr>
	<tr class='pair'>
		...
		<td><input type='checkbox' value='1' name='fromdraft'></td>
		
	</tr>
	<tr class='odd'>
		<td colspan=3  style='text-align: center'><button class='button button_red_w' id='b_copy'>Copy From</button>
<input type='text' name='c_start' id='c_inicio' value='<?php echo $c_inicio; ?>'><input type='text' name='c_end' id='c_fin' value='<?php echo $c_fin; ?>'></td>
	</tr>
	
<input type='hidden' value='0' name='copy' id='copy'>
</form></table>
<br><br>
