...
foreach($data as $id => $info){
	$query="INSERT INTO `Historial Programacion` VALUES (NULL,'".$info['id']."',NULL,'$fecha','".$info['j_s']."','".$info['j_e']."','".$info['c_s']."','".$info['c_e']."','".$info['x1_s']."','".$info['x1_e']."','".$info['x2_s']."','".$info['x2_e']."','0')";
	if($result=$connectdb->query($query)){
		$td[$id]['status']='inserted';
		$td[$id]['msg']='ok';
	}else{
		$td[$id]['status']=$connectdb->error." ON $query";
		$query="UPDATE `Historial Programacion` SET `jornada start`='".$info['j_s']."', `jornada end`='".$info['j_e']."', "
				...
				." WHERE Fecha='$fecha' AND asesor='".$info['id']."'";
