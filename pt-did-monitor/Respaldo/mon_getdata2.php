...
//Volumen
	$query="SELECT IF(Canal IS NULL,'Xfer',Canal) as Canal, SUM(IF(Calls IS NULL,0,Calls)+IF(Unanswered IS NULL,0,Unanswered)) as Llamadas, MAX(Last_Update) as lu 
			...
			WHERE a.Skill IN (30,35) AND a.Fecha BETWEEN '$date' AND '$dateto' AND ((a.Did='Untracked' AND Skill=35) OR Grupo_Canal LIKE '%MP%') 
			GROUP BY b.Canal";
...
//Forecast
$query="SELECT hora, participacion*volumen as fc FROM forecast_participacion a LEFT JOIN forecast_volume b ON a.Fecha=b.Fecha AND a.skill=b.skill  WHERE a.Fecha BETWEEN '$date' AND '$dateto' AND a.Skill=35 ORDER BY hora";
if($result=$connectdb->query($query)){
	while($fila=$result->fetch_assoc()){
...
$query="SELECT 
        ...
        LEFT JOIN 
          (SELECT 
          a.id, Hora, Desconexion, b.*, IF(dep=29 AND puesto!=1,'Desborde',NULL) as desborde
        FROM
          monptdid a
        LEFT JOIN
          daily_dep b ON asesorID!=0 AND a.asesorID=b.asesor) a ON a.Hora BETWEEN c.Hora_time AND ADDTIME(c.Hora_time,'00:29:29') 
...
//Calls Hour LY
$query="SELECT Hora_int, COUNT(*)as Calls FROM HoraGroup_Table c LEFT JOIN (SELECT a.ac_id, Hora FROM t_Answered_Calls a LEFT JOIN Cola_Skill b ON a.Cola=b.Cola WHERE Fecha=ADDDATE('$date',-371) AND Skill=35) a ON a.Hora BETWEEN c.Hora_time AND ADDTIME(c.Hora_time,'00:29:29') GROUP BY Hora_group ORDER BY Hora_int";
if($result=$connectdb->query($query)){
		while($fila=$result->fetch_assoc()){
...
//Calls Hour Yd
$query="SELECT Hora_int, COUNT(*)as Calls FROM HoraGroup_Table c LEFT JOIN (SELECT a.ac_id, Hora FROM t_Answered_Calls a LEFT JOIN Cola_Skill b ON a.Cola=b.Cola WHERE Fecha=ADDDATE('$date',-1) AND Skill=35) a ON a.Hora BETWEEN c.Hora_time AND ADDTIME(c.Hora_time,'00:29:29') GROUP BY Hora_group ORDER BY Hora_int";
if($result=$connectdb->query($query)){
		while($fila=$result->fetch_assoc()){
...
//Calls Hour LW
$query="SELECT Hora_int, COUNT(*)as Calls FROM HoraGroup_Table c LEFT JOIN (SELECT a.ac_id, Hora FROM t_Answered_Calls a LEFT JOIN Cola_Skill b ON a.Cola=b.Cola WHERE Fecha=ADDDATE('$date',-7) AND Skill=35) a ON a.Hora BETWEEN c.Hora_time AND ADDTIME(c.Hora_time,'00:29:29') GROUP BY Hora_group ORDER BY Hora_int";
if($result=$connectdb->query($query)){
		while($fila=$result->fetch_assoc()){
