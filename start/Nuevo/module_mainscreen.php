...
class mainScreen{
	...
	public function __construct($asesor){
		...
		$query="SELECT
            Nombre, Esquema, num_colaborador, id, RFC, telefono1, telefono2, correo_personal, Vigencia_Pasaporte, Vigencia_Visa, Locker, `id Departamento` as departamento
        FROM
            Asesores a 
        LEFT JOIN
	...
	public function showFilter(){
		...
		echo "<table style='width:600px; margin: auto; text-align: center;'><tr><th>Departamento</th><th><select name='dep' id='departamentoselection'><option value=''>Selecciona...</option>";
		...
		echo "</select></th><th>Asesor</th><th><select name='asesor' id='asesorselection'><option value=''>Selecciona...</option>";
		
		$query="SELECT id, `N Corto`, `id Departamento` FROM Asesores WHERE Activo=1 ORDER BY `N Corto`";
		if($result=Queries::query($query)){
			while($fila=$result->fetch_assoc()){
				echo "<option value='".$fila['id']."' class='depsel_".$fila['id Departamento']." depselection' ";
				if($fila['id']==$_POST['asesor']){ echo "selected"; }
		...
		echo "</select></th><th><button class='button button_red_w' id='search'>Ver</button></th></tr></table></form></div>";
