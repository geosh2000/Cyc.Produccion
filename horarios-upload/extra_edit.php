...
for($i=0;$i<$num;$i++){
	$data[mysql_result($result,$i,'Fecha')]['factor']=mysql_result($result, $i, 'factor');
	$data[mysql_result($result,$i,'Fecha')]['volumen']=mysql_result($result, $i, 'volumen');
	$data[mysql_result($result,$i,'Fecha')]['AHT']=mysql_result($result, $i, 'AHT');
	$data[mysql_result($result,$i,'Fecha')]['Reductores']=mysql_result($result, $i, 'Reductores');
}

$query="SELECT Departamento FROM PCRCs WHERE id=$skill";
$result=mysql_query($query);
$depart=mysql_result($result, 0, 'Departamento');
