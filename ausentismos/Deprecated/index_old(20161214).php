...
$motivo=$_POST['motivo'];
if(isset($asesor)){
	if($result=$connectdb->query("SELECT `N Corto` FROM Asesores WHERE id='$asesor'")){
		$fila=$result->fetch_assoc();
		$asesor_name=$fila['N Corto'];
	}
}
if(isset($ausentismo)){
	if($result=$connectdb->query("SELECT Ausentismo FROM `Tipos Ausentismos` WHERE id='$ausentismo'")){
		$fila=$result->fetch_assoc();
		$ausentismo_name=$fila['Ausentismo'];
	}
}
...
//Function for listing Days
	function print_days($dept){
		global $asesor, $motivo, $ausentismo, $aus_days, $aus_id, $dias, $connectdb;
		$as=$asesor;
		$optprint="<option value=''>Selecciona...</option>";
		if($ausentismo!=5){
			$i=1;
...
//Function Motivos por asesor para Dias Pendientes
	...
	global $motivo;
		...
		while($i<mysql_numrows($result)){
			...
			echo "\t<option value='".utf8_encode(mysql_result($result,$i,'motivo'))."' $as>".utf8_encode(mysql_result($result,$i,'motivo'))."</option>\n";
		$i++;
		}
		echo "</select></td></tr>";
...
<script>
  $(function() {
    ...
    $( '#1000, zh:gt(0):lt(5000)' ).button().on( "click", function() {
        ...
        var x_delid=document.getElementById('del_id');
        x_delid.innerText= fx_id.innerText;
...
<table width='100%' class='t2'><form name='aus' method='post' action='<?php $_SERVER['PHP_SELF']; ?>'>
	...
	<tr class='title'>
		...
		<td  class='pair' width='25%'><select name='dep' id="pcrc_select" required><?php echo $departs; ?></select></td>
		<td width='25%'>Asesor:</td>
		<td class='pair' width='25%'><select name='asesor' id="sel" onchange='this.form.submit()' required><option value=''>Selecciona...</option><?php
        $query="SELECT * FROM Asesores WHERE Activo=1 ORDER BY `N Corto`";
        $result=mysql_query($query);
        $num=mysql_numrows($result);
        for($i=0;$i<$num;$i++){
            if(mysql_result($result,$i,'id')==$asesor){$selected=' selected';}else{$selected="";}
            echo "<option value='".mysql_result($result,$i,'id')."' class='optasesores optdep".mysql_result($result,$i,'id Departamento')."' $selected>".mysql_result($result,$i,'N Corto')."</option\n\t>";
        }
...
while($i<$num){
    ...
    $a_tipo[$i]=mysql_result($result,$i,'Ausentismo');
     ...
     $a_dias[$i]=(strtotime($a_ff[$i])-strtotime($a_fi[$i]))/60/60/24-$a_beneficios[$i]-$a_descansos[$i]+1;
...
<?php if($ausentismo==1 || $ausentismo==2 || $ausentismo==3 || $ausentismo==9 || $ausentismo==5){
...
<div id="dialog-confirm2" title="Eliminar Ausentismo">
  <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>El AUSENTISMO id:<dh id='del_id'></dh> se eliminar� del registro. Estas seguro?</p>
</div>
