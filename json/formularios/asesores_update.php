...
if(!isset($_SESSION['id'])){
  ...
  $data['status']=0;
  $data['msg']=utf8_encode('Sesion expirada, por favor inicia sesion nuevamente');
...
$oldVal="'".utf8_decode($_POST['oldVal'])."'";
...
switch($field){
	case "pswd":
		...
		$histo="INSERT INTO historial_asesores (id, asesor, campo, old_val, new_val, changed_by) VALUES (NULL, $id, 'password','','reseted',".$_SESSION['asesor_id'].")";
		break;
	case "profile":
		$query="UPDATE userDB SET profile=$newVal WHERE asesor_id= $id ";
		$histo="INSERT INTO historial_asesores (id, asesor, campo, old_val, new_val, changed_by) VALUES (NULL, $id, 'profile',$oldVal,$newVal,".$_SESSION['asesor_id'].")";
		break;
	default:
		$query="UPDATE `Asesores` SET `$field`=$newVal WHERE id='$id'";
		$histo="INSERT INTO historial_asesores (id, asesor, campo, old_val, new_val, changed_by) VALUES (NULL, $id, '$field',$oldVal,$newVal,".$_SESSION['asesor_id'].")";
...
if($data['status']==1){
  if(!$result=$connectdb->query($histo)){
    $data['status']=0;
    $data['msg']=utf8_encode('No se guardo historial -> '.$connectdb->error);
  }
}
