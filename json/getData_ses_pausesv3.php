...
function tzChange($time, $date, $oirg, $new){
	...
	$original -> setTimezone($GLOBALS['cun_time']);
...
if(!isset($_GET['date']) || $_GET['date']==NULL){$date=date('Y-m-d');}else{$date=date('Y-m-d',strtotime($_GET['date']));}
$asesor=$_GET['asesor'];
...
if($result=Queries::query($query)){
	while($fila=$result->fetch_assoc()){
		...
		$time -> setTimezone($GLOBALS['cun_time']);
