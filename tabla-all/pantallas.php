...
//query today info
$query="SELECT
        		COUNT(IF((PCRC=3 OR PCRC=4 OR PCRC=6) OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND PCRC IS NULL),Monto,NULL)) as InboundAllLocs,
        		COUNT(IF(((Afiliado LIKE'%pricetravel.com.mx%' OR Afiliado LIKE'%Cerrados%') AND (PCRC=3 OR PCRC=4 OR PCRC=6)) OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND PCRC IS NULL),Monto,NULL)) as InboundMPLocs,
        		...
        		(SUM(IF((PCRC=3 OR PCRC=4 OR PCRC=6) OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND PCRC IS NULL),Monto,NULL)) * Dolar) as InboundAllMonto,
        		SUM(IF(((Afiliado LIKE'%pricetravel.com.mx%' OR Afiliado LIKE'%Cerrados%') AND (PCRC=3 OR PCRC=4 OR PCRC=6)) OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND PCRC IS NULL),Monto,NULL)) * Dolar as InboundMPMonto,
    ...
    LEFT JOIN
		(
			SELECT
				...
				COUNT(IF(Canal='MP MX',ac_id,NULL)) as LlamadasMP,
				COUNT(IF(Canal='Intertours',ac_id,NULL)) as LlamadasIT,
				COUNT(IF(Canal='COPA',ac_id,NULL)) as LlamadasCOPA,
				COUNT(IF(Canal='COOMEVA',ac_id,NULL)) as LlamadasCOOMEVA
			FROM
				Fechas d
			JOIN
				t_Answered_Calls a
