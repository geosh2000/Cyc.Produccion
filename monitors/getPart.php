...
$query="DROP TEMPORARY TABLE IF EXISTS mon_part_calls;

        CREATE TEMPORARY TABLE mon_part_calls (SELECT a.*, IF(SUBSTR(Agente,1,LOCATE('(', Agente)-2)='',NULL,getIdAsesor(SUBSTR(Agente,1,LOCATE('(', Agente)-2),2)) as asesorID
            FROM
              (SELECT a.*, Skill 
                FROM ccexporter.mon_calls_details a 
                LEFT JOIN
                      Cola_Skill b ON a.Cola=b.Cola 
                WHERE Fecha=CURDATE() AND Desconexion NOT IN ('Abandon','Unanswered yet') 
                HAVING Skill=35
              ) a );";
...
$query="SELECT GrupoCC, locs.*, calls.llamadas, locs.Locs/llamadas*100 as FC
        ...
        FROM
          ...
          FROM
            mon_part_calls a
          LEFT JOIN
            daily_dep b ON a.asesorID=b.asesor
          LEFT JOIN
            cc_apoyo c ON a.asesorID IS NOT NULL AND a.asesorID=c.asesor AND CURDATE() BETWEEN inicio AND fin) a
        GROUP BY
        GrupoCC) calls
        LEFT JOIN
        (SELECT
        ...
        LEFT JOIN daily_dep c ON a.asesor=c.asesor
        WHERE Fecha=CURDATE() AND (chanId IN(1,2,3,4,5,11,309,332) OR (chanId=25 AND tipo=2)) HAVING dep NOT IN (5)) a
        GROUP BY Grupo) locs
