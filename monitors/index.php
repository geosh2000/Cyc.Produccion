...
class liveMonitor{
	...
	public function __construct($skill){
		...
		$query="SELECT `N Corto`, Departamento FROM (SELECT `N Corto`, getDepartamento(id,CURDATE()) as Dep FROM Asesores HAVING Dep NOT IN (29,31) AND Dep IS NOT NULL) a LEFT JOIN PCRCs b ON a.Dep=b.id";
		if($result=$tmpconnect->query($query)){
			while($fila=$result->fetch_assoc()){
	...
	public function printBlocks($tipo){
		...
		switch($tipo){
			...
			case 'v':
				...
				break;
			case 'all':
				echo "<style>body{ zoom: 0.45; }</style>";
				echo $bloques;
				break;
			case 'queues':
				echo "<style>body{ zoom: 0.7; }</style>";
