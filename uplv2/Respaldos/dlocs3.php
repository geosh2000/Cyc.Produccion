...
foreach($data as $key1 => $row){
	...
	if($q_var!=""){
		...
		//MySQLi
		$queryUD.="UPDATE $db SET $q_ins WHERE locs_id=$md_id; ";
		$queryINS.="INSERT INTO $db ($q_var) VALUES ($q_val); ";
...
$mysqli    = new mysqli("67.225.221.130", $username, $password, $database);

//UPDATE
		if (!$mysqli->multi_query($queryUD)) {
		...
		//INSERT
		if (!$mysqli->multi_query($queryINS)) {
		    echo "Falló la multiconsulta: (" . $mysqli->errno . ") " . $mysqli->error;
		}
