...
$query="SELECT
	Fecha, id, `N Corto`, IF(LocsT IS NULL, LocsD, LocsT) as Localizadores, IF(t_Monto IS NULL,d_Monto,t_Monto)*1 as Monto, IF(llamadas IS NULL,dllamadas,llamadas) as llamadas,
    Aus, DiasHabiles
FROM
	(
		SELECT
...
LEFT JOIN
	(
		SELECT
			Fecha as calls_Fecha, asesor as calls_asesor, count(IF(Answered=1,ac_id,NULL))-IF(COUNT(IF(Desconexion='Transferida' AND Duracion_Real<='00:01:00',ac_id,NULL)) IS NULL, 0,COUNT(IF(Desconexion='Transferida' AND Duracion_Real<='00:01:00',ac_id,NULL))) as llamadas
		FROM
			t_Answered_Calls a
		LEFT JOIN
			Cola_Skill b
...
//Calculated Fields
for($i=0;$i<$row_cnt;$i++){
	if(date("I", strtotime($fecha[$i]))==1){$minus="4";}else{$minus="5";} //Convert Date for HighCharts
	$datetmp=strftime("%s", strtotime($data['Fecha'][$i].' -'.$minus.' hours'))*1000;
	$xdata[$i]=$datetmp;
	
	$data['locs'][$i]=array($datetmp,intval($data['Localizadores'][$i]));
	@$data['metadiaria'][$i]=array($datetmp,intval($mt/intval($data['DiasHabiles'][$i])));
	
	$acumulado['Monto']+=$data['Monto'][$i];
    $acumulado['Localizadores']+=$data['Localizadores'][$i];
    $acumulado['Llamadas']+=$data['llamadas'][$i];
    
    @$data['acumfc'][$i]=array($datetmp,$acumulado['Localizadores']/$acumulado['Llamadas']*100);
    @$data['acummonto'][$i]=array($datetmp,intval($acumulado['Monto']));
    @$data['arfc'][$i]=array($datetmp,intval($data['Localizadores'][$i]/$data['llamadas'][$i]*100));
    @$data['armonto'][$i]=array($datetmp,intval($data['Monto'][$i]));
    
    if($data['Aus'][$i]==1){
        $acumulado['md']+=intval($mt/intval($data['DiasHabiles'][$i]));
    }
    @$data['metadiaria'][$i]=array($datetmp,intval($mt/intval($data['DiasHabiles'][$i])));
    @$data['mdtacum'][$i]=array($datetmp,intval($acumulado['md']));
}
