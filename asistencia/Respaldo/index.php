...
FROM
(SELECT a.id, c.Fecha, Nombre, num_colaborador, IF(getDepartamento(a.id, c.Fecha) IS NULL, getLastDep(a.id), getDepartamento(a.id, c.Fecha)) as dep,
IF(getPuesto(a.id, c.Fecha) IS NULL, getLastPuesto(a.id), getPuesto(a.id, c.Fecha)) as pues, Egreso, Esquema FROM Asesores a JOIN (SELECT Fecha FROM Fechas WHERE Fecha BETWEEN '$from' AND '$to') c WHERE (Egreso >= '$from' AND Egreso IS NOT NULL) HAVING $sel_dep ORDER BY Nombre) a LEFT JOIN PCRCs b ON a.dep=b.id  LEFT JOIN PCRCs_puestos c ON a.pues=c.id";
if($result=$connectdb->query($query)){
