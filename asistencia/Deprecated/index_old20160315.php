...
$query="SELECT
	Esquema, Fecha, if(DAYOFWEEK(Fecha)=1,7,DAYOFWEEK(Fecha)-1) as DOW, DATEDIFF(Fin, Inicio) as d_ausentismo, id_asesor, id_Dep, Departamento, `N Corto`, Nombre, Ingreso, id_hora, `jornada start`, `jornada end`,
	...
	FROM
	(SELECT
	Dates.Esquema as Esquema, Dates.Fecha as Fecha, Dates.id as id_asesor, Dates.`id Departamento` as id_Dep, Dates.`Departamento` as Departamento, Ingreso, Dates.`N Corto` as 'N Corto', Dates.Nombre as Nombre, Histo.id_hora, Histo.`jornada start`as 'jornada start',
	Histo.`jornada end` as 'jornada end'
	FROM
		(
		SELECT
		Fecha, id, `N Corto`, Nombre, `id Departamento`, Departamento, Ingreso, Esquema
		FROM
			Fechas
		JOIN
			(SELECT a.id, `N Corto`, Nombre, `id Departamento`, Departamento, Activo, Ingreso, Esquema FROM Asesores a, PCRCs b WHERE a.`id Departamento`=b.id) as Asesores
		WHERE
			Fecha>='$from' AND
			Fecha<='$to' AND
			Activo=1 $sel_dep
