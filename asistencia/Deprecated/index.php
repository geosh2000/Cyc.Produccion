...
FROM
(SELECT a.id, c.Fecha, Nombre, num_colaborador, getDepartamento(a.id, c.Fecha) as dep, Egreso, Esquema FROM Asesores a JOIN (SELECT Fecha FROM Fechas WHERE Fecha BETWEEN '$from' AND '$to') c WHERE (Egreso >= '$from' AND Egreso IS NOT NULL) HAVING $sel_dep ORDER BY Nombre) a LEFT JOIN PCRCs b ON a.dep=b.id";
if($result=$connectdb->query($query)){
	...
	while($fila=$result->fetch_row()){
		...
		$depAsesor[$fila[0]][utf8_encode($fila[7])]=$fila[7];
	}
}else{
