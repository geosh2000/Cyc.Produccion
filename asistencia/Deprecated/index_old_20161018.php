...
<?php
if($_SESSION['monitor_pya_exceptions']==1){include("../common/add_exception_test.php");}
...
<table style='width:600px; margin:auto' class='t2'><form action="<?php $_SERVER['PHP_SELF'] ?>" method='post'>
    ...
    <tr class='title'>
        ...
        function list_deps(){
            global $p_dep;
            $query="SELECT * FROM PCRCs WHERE parent=1 AND Departamento NOT LIKE '%pdv%' AND Sede='MX' ORDER BY Departamento";
        ...
        <td class='pair' style='text-align:right'><label for="showh">Horarios</label><input type="checkbox" id="showh" name="showh" <?php echo $showh ?>><br>
                        ...
                        <label for="showret">Retardos</label><input type="checkbox" id="showret" name="showret" <?php echo $showret ?>><br>
                        <label for="showcom">Comidas</label><input type="checkbox" id="showcom" name="showcom" value='1' <?php echo $showcom ?>>
        </td>
...
<table width=100% id='horarios' class='t2' style='text-align: center;'>
    <thead>
    <tr>
        <th>Asesor</th><th>Colaborador</th><th>Departamento</th>
        <?php
...
`N Corto`,
	...
	IF(LogAsesor(Fecha, id, 'in')>'21:00:00' AND `jornada start`='00:00:00','00:00:00',LogAsesor(Fecha, id, 'in')) as Hora_in,
	LogAsesor(Fecha, id, 'out') as Hora_out,
	`jornada start`,`jornada end`,`comida start`, `comida end`,
	Inicio, Fin, Descansos, Beneficios, Ausentismo, Code, Comments, aus_user,
	pya_caso as caso, Nota, Codigo,
	IF(`jornada start`!=`jornada end`!='00:00:00',(TIME_TO_SEC(LogAsesor(Fecha, id, 'in'))-TIME_TO_SEC(`jornada start`))/60,NULL) as Retardos,PyAAus, pya_user, pya_caso
	...
	JOIN
		(
			...
			WHERE
				Activo=1 AND Sede='MX' AND Departamento NOT LIKE '%pdv%' $sel_dep
		) Asesores
	LEFT JOIN
		(
			SELECT
				Fecha as Programacion_Fecha, id as Programacion_id, asesor as Programacion_asesor, `jornada start`, `jornada end`, `comida start`, `comida end`
			FROM
...
while($i<$num){
    ...
    $colaborador[$y]=mysql_result($result,$i,'num_colaborador');
    ...
    while(mysql_result($result,$i,'Nombre') == $asesor[$y]){
        ...
        $comida_in[$y][$x]=mysql_result($result,$i,'comida start');
        $comida_out[$y][$x]=mysql_result($result,$i,'comida end');
...
foreach($asesor as $key => $name){
    ...
    echo "<tr><td style='text-align:left;'>$name</td><td>$colaborador[$key]</td><td>$dep[$key]</td>";
    ...
    while($x<$numf){
        ...
        if($showh=="checked"){$title.="Comida: ".$comida_in[$key][$x]." - ".$comida_out[$key][$x]." // ";}
        ...
        if($horario_in[$key][$x]=="00:00:00" && $horario_out[$key][$x]=="00:00:00"){
            ...
            $tmp_turno="Descanso";}else{$tmp_turno=date('H:i',strtotime($horario_in[$key][$x]))." - ".date('H:i',strtotime($horario_out[$key][$x]));}
        ...
        if($showh=="checked"){
        	if($showcom=="checked"){
        		$show="$tmp_turno<br>C: (".date('H:i',strtotime($comida_in[$key][$x]))." - ".date('H:i',strtotime($comida_out[$key][$x])).")<br>"; $row1 .=$tmp_turno.$separator;
			}else{
				$show="$tmp_turno<br>"; $row1 .=$tmp_turno.$separator;
			}
		}else{
			$show="";
		}
        ...
        echo "\t\t<td title='$title'><sh style='width:100%' class='$tmp_class addexcep' id='$z' ncorto='$corto[$key]' horaid='".$hid[$key][$x]."' nameid='$as_id[$key]' fecha='".$fecha[$key][$x]."'>$show $exce $sret<z id='a$z'></z></sh></td>\n";
