...
if($dept==3){$dept='3,35';}else{$dept=$dept;}
	...
	LEFT JOIN
		(
			SELECT
				...
				COUNT(DISTINCT IF(`id Departamento` IN ($dept),asesor,NULL)) as AsesoresPrimerNivel,
				COUNT(DISTINCT IF(`id Departamento` NOT IN ($dept) or `id Departamento` IS NULL,asesor,NULL)) as AsesoresApoyo
			FROM
				t_Answered_Calls a
			LEFT JOIN
				Cola_Skill b
	...
	LEFT JOIN
		(
			SELECT
				...
				COUNT(IF(`id Departamento` IN ($dept) OR (`id Departamento` IS NULL AND Afiliado LIKE '%agentes.pricetravel.com.mx%') OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND `id Departamento`!=5),localizador,NULL)) as LocalizadoresTotal,
				SUM(IF(`id Departamento` IN ($dept) OR (`id Departamento` IS NULL AND Afiliado LIKE '%agentes.pricetravel.com.mx%') OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND `id Departamento`!=5),Monto,NULL)) as MontoTotal,
				COUNT(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%pricetravel.com.mx%') OR (`id Departamento` IS NULL AND Afiliado LIKE '%agentes.pricetravel.com.mx%') OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND `id Departamento`!=5),localizador,NULL)) as LocalizadoresMP,
				SUM(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%pricetravel.com.mx%') OR (`id Departamento` IS NULL AND Afiliado LIKE '%agentes.pricetravel.com.mx%') OR (Afiliado LIKE'%agentes.pricetravel.com.mx%' AND `id Departamento`!=5),Monto,NULL)) as MontoMP,
				COUNT(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%intertours%'),localizador,NULL)) as LocalizadoresIT,
				SUM(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%intertours%'),Monto,NULL)) as MontoIT,
				COUNT(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%copa%'),localizador,NULL)) as LocalizadoresCOPA,
				SUM(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%copa%'),Monto,NULL)) as MontoCOPA,
				COUNT(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%coomeva%'),localizador,NULL)) as LocalizadoresCOOMEVA,
				SUM(IF((`id Departamento` IN ($dept) AND Afiliado LIKE '%coomeva%'),Monto,NULL)) as MontoCOOMEVA
	...
	LEFT JOIN
		(
			SELECT
				...
				SUM(IF(Skill IN ($dept) AND Productiva=0,TIME_TO_SEC(Duracion),0)) as PNP,
				SUM(IF(Skill IN ($dept) AND Productiva=1,TIME_TO_SEC(Duracion),0)) as PP
			FROM
				t_pausas a
			LEFT JOIN
				Tipos_pausas b
	...
	LEFT JOIN
		(
			SELECT
				...
				SUM(IF(Skill IN ($dept),TIME_TO_SEC(Duracion),0)) as DuracionSesiones
			FROM
				t_Sesiones
			WHERE
