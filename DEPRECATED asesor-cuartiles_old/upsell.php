...
$query="SELECT
	...
	LEFT JOIN
		(
			SELECT
				asesor as Localizadores_asesor, Fecha as Localizadores_Fecha, COUNT(*) as Locs, SUM(Monto) as MontoPorDia
			FROM
				(
					SELECT
						asesor, Fechas.Fecha, Localizador, SUM(Venta+OtrosIngresos+Egresos)*Dolar as Monto
					FROM
						Fechas
					LEFT JOIN
						t_Locs
	...
	LEFT JOIN
		(
			SELECT
				asesor as LocsMonto_asesor, Fecha as LocsMonto_Fecha, SUM(Monto) as MontoAsesor
			FROM
				(
					SELECT
						asesor, Fechas.Fecha, Localizador, SUM(Venta+OtrosIngresos+Egresos)*Dolar as Monto
