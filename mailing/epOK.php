...
function sendMail($user, $m_data){
                                                                       ...
                                                                       <tbody>
                                                                          <tr>
                                                                             <td valign='top' style='padding-top: 9px;padding-right: 18px;padding-bottom: 9px;padding-left: 18px;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;color: #606060;font-family: Helvetica;font-size: 15px;text-align: center;'>
                                                                                <p><span style='color: #5266a2; font-size: 24px; line-height: 130%;'>¡Hola $name!<br><br>Tu solicitud de Cambio/Baja se encuentra en proceso de Revisión</span></p>
                                                                                <p style='line-height: 140%;'> El usuario <em><span style='color: #5266a2;'><strong>".$m_data['sol']."</strong></span>&nbsp;</em>ha cambiado el status a \"En Proceso\"<br><br><span style='text-decoration: underline; color: #5266a2;'><strong>Asesor: ".$m_data['asesor']."<br></strong></span></p>
                                                                                <p style='line-height: 140%;'>Notas registradas: \"".utf8_encode($m_data['comentarios'])."\"</strong></p>
  ...
  mail("$user@pricetravel.com","Solicitud en Proceso de Revisión (".$m_data['usuario'].")",$msg,$headers);
    
    $td['mails_sent'][]=utf8_encode("$user@pricetravel.com");
...
$query="SELECT NombreAsesor(".$_SESSION['asesor_id'].",1) as updater, NombreAsesor($solicitante,1) as sol, NombreAsesor(".$input['asesor'].",1) as asesor";
...
$query="SELECT usuario FROM mail_lists WHERE notif='epOK'";
if($result=Queries::query($query)){
