...
function sendMail($user, $cantidad, $m_data){
                                                                       ...
                                                                       <tbody>
                                                                          <tr>
                                                                             <td valign='top' style='padding-top: 9px;padding-right: 18px;padding-bottom: 9px;padding-left: 18px;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;color: #606060;font-family: Helvetica;font-size: 15px;text-align: center;'>
                                                                                ...
                                                                                <p style='line-height: 140%;'> El usuario <em><span style='color: #5266a2;'><strong>".$m_data['sol']."</strong></span>&nbsp;</em>ha solicitado <strong><span style='color: #5266a2;'>$cantidad</span>&nbsp;</strong>nuevas vacantes para:<br><span style='text-decoration: underline; color: #5266a2;'><strong>".utf8_encode($m_data['dep'])." -&gt;&nbsp;".utf8_encode($m_data['puesto'])."</strong>
                                                                                <br>La ubicación es: ".utf8_encode($m_data['ciudad'])." -> ".utf8_encode($m_data['oficina'])."</span></p>
  ...
  mail("$user@pricetravel.com","Nueva Vacante Solicitada (".$m_data['sol'].")",$msg,$headers);
  
}

$query="SELECT NombreAsesor(".$_SESSION['asesor_id'].",1) as name, Departamento FROM PCRCs WHERE id=$dep";
if($result=Queries::query($query)){
  $fila=$result->fetch_assoc();
  $m_data['dep']=$fila['Departamento'];
  $m_data['sol']=$fila['name'];
}
...
$query="SELECT Ciudad FROM db_municipios WHERE id=$ciudad";
if($result=Queries::query($query)){
  $fila=$result->fetch_assoc();
  $m_data['ciudad']=$fila['Ciudad'];
}

$query="SELECT PDV FROM PDVs WHERE id=".$_POST['pdv'];
if($result=Queries::query($query)){
  $fila=$result->fetch_assoc();
  $m_data['oficina']=$fila['PDV'];
}

$query="SELECT usuario FROM mail_lists WHERE notif='vacantes'";
if($result=Queries::query($query)){
  while($fila=$result->fetch_assoc()){
     sendMail($fila['usuario'],$cantidad,$m_data);
  }
}
