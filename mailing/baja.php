...
function sendMail($user, $m_data){
                                                                       ...
                                                                       <tbody>
                                                                          <tr>
                                                                             <td valign='top' style='padding-top: 9px;padding-right: 18px;padding-bottom: 9px;padding-left: 18px;mso-table-lspace: 0pt;mso-table-rspace: 0pt;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;color: #606060;font-family: Helvetica;font-size: 15px;text-align: center;'>
                                                                                <p><span style='color: #5266a2; font-size: 24px; line-height: 130%;'>¡Hola $name!<br>Se ha registrado una Baja</span></p>
                                                                                <p style='line-height: 140%;'> El usuario <em><span style='color: #5266a2;'><strong>".$m_data['sol']."</strong></span>&nbsp;</em>ha registrado la siguiente baja:<br><span style='text-decoration: underline; color: #5266a2;'><strong>".utf8_encode($m_data['usuario'])."<br>Ultimo puesto: ".utf8_encode($m_data['dep'])." -&gt;&nbsp;".utf8_encode($m_data['puesto'])."</strong>
                                                                                <br>con fecha: <strong>".$m_data['fecha_baja'])."</strong></span></p>";
                                                                                
                                                                                if($m_data['replace']==1){
                                                                                  $msg.="<p style='line-height: 140%;'>La vacante queda liberada con fecha <strong>".$m_data['fecha']."</strong></p>";
                                                                                }else{
                                                                                  $msg.="<p style='line-height: 140%;'>La vacante ha quedado inactiva por lo que no necesita reemplazo</strong></p>";
                                                                                }
  ...
  mail("$user@pricetravel.com","Baja Registrada (".$m_data['usuario'].")",$msg,$headers);
  
}

$query="SELECT NombreAsesor(".$_SESSION['asesor_id'].",1) as sol, NombreAsesor(".$input['asesor'].",1) as asesor";
...
$query="SELECT b.Departamento, c.Puesto FROM asesores_plazas a LEFT JOIN PCRCs b ON a.departamento=b.id LEFT JOIN PCRCs_puestos c ON a.puesto=c.id WHERE a.id=".$vac_off;
if($result=Queries::query($query)){
  $fila=$result->fetch_assoc();
  $m_data['dep']=$fila['Departamento'];
  $m_data['puesto']=$fila['Puesto'];
}

$m_data['replace']=$input['replace'];
$m_data['fecha']=$input['fecha_out'];
$m_data['fecha_baja']=$input['fecha_in'];

$query="SELECT usuario FROM mail_lists WHERE notif='bajas'";
if($result=Queries::query($query)){
