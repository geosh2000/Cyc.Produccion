...
*/
    ...
    function validate(){
        ...
        if(flag_canal && flag_producto && flag_mg && flag_me && flag_detalle && flag_other){flag=true;}else{flag=false;}
             ...
             switch (valor){
                 case '2':
                    ...
                    $('#contain-detalle').show('200').attr('req',1);
                     ...
                     $('.detalle7').show('200').attr('req',1);
                    break;
                 case '3':
                    $('#f_me').val('6');
                    $('#contain-detalle').show('200').attr('req',1);
                    $('.id_detalle').hide('200').attr('req',0);
                     $('.detalle6').show('200').attr('req',1);
                    break;
                 default:
                    $('#f_me').val('');
    ...
    $('#submit_form').click(function(){
        validate();
        if(flag){
            var variables="asesor=<?php echo $asesor; ?>&canal=" + $('#f_canal').val() + "&producto=" + $('#f_producto').val() + "&mg=" + $('#f_mg').val() + "&me=" + $('#f_me').val() + "&detalle=" + $('#f_detalle').val() + "&otro=" + $('#f_other').val();
            //alert(variables);
    ...
    <div id='contain-detalle' class='campo'>
        ...
        <div class='opcion'>
            <p class='seleccion'><select id='f_detalle' class='input'><option value=''>Selecciona...</option><?php
                ...
                $i=0;
                while($i<$num){
                    echo "<option class='id_detalle detalle".mysql_result($result,$i,'motivo_especifico')."' value='".mysql_result($result,$i,'id')."'>".utf8_encode(mysql_result($result,$i,'Detalle'))."</option>";
                $i++;
                }
            ?></select>*</p>
        </div>
