...
$(function(){
	...
	meta['ol']=12525964;
	meta['PDV']=10835370;
	meta['PTMX']=meta['ibMP']+meta['us']+meta['ol'];
	meta['All']=7500000;
	//meta['All']=4000000;
	...
	bars.yAxis[0].removePlotLine('meta-1');
    	redrawChart(data);
    });
    
    function redrawChart(info){
    	bars.series[0].setData(data['TD'][$('#canal').val()]);
        bars.series[1].setData(data['YD'][$('#canal').val()]);
        bars.series[2].setData(data['LW'][$('#canal').val()]);
        bars.series[3].setData(data['LY'][$('#canal').val()]);
        //bars.series[1].setData(data['1bf'][$('#canal').val()]);
        //bars.series[4].setData(data['2bf'][$('#canal').val()]);
        bars.setTitle({text: $('#canal option:selected').text()});
        
        //PlotLine META
        if($('#canal').val()=='ibMP' || $('#canal').val()=='All'){
	        bars.yAxis[0].addPlotLine({
	                value: meta[$('#canal').val()],
	                color: 'red',
	                width: 3,
	                id: 'meta-1'
	            });
	            
		if(data['TD'][$('#canal').val()]>=meta[$('#canal').val()]){
			tope=null;
		}else{
			tope=meta[$('#canal').val()]+1000000;
		}
...
<div style='text-align: right;'>
	...
	<button class='buttonlarge button_red_w' id='productos'>Ver Productos</button>
	<p id='lu'></p>
