...
if(isset($_GET['s'])){$departamento=$_GET['s'];}else{$departamento=$_SESSION['dep'];}
$url=$_GET['url'];
...
if(isset($_GET['caller']) || $_GET['caller']!=""){
    ...
    preg_match("/(?<=NewCall\?caller\=).*/", $url, $tmp_caller);
    $caller=$tmp_caller[0];
}
...
if($_SESSION['dep']==NULL){$flag_ses='no';}else{$flag_ses='ok';}
...
if(($departamento!=4 && $departamento!=1 && $departamento!=2) || $_SESSION['dep']==NULL){
    header("Location: $url");
    exit;
}

$createReg="INSERT INTO fcr (date_created, asesor,departamento,caller) VALUE ('$created', '".$_SESSION['asesor_id']."','".$_SESSION['dep']."','$caller')";
...
function checkLog(flag){
    if(flag=='no'){
        new noty({
                    text: 'Tu sesion de ComeyCome no esta activa. Por favor logueate nuevamente',
                    type: 'error',
                    timeout: 10000,
                    animation: {
...
$('#ic_search').click(function(){
    window.open('https://rsv.pricetravel.com.mx/reservations/show/'+$('#localizador').val());
});
...
<input style='width:115' type='text' id='localizador' value='' name='localizador'> <img id='ic_search' src="../images/search.png" alt="ok" height="10" width="10">
