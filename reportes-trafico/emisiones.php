...
if(isset($_POST['submit'])){
	...
	$query="SELECT 
				a.id, b.gds, 
				if(tipo=1,'Emision',if(tipo=2,'Revisado',if(tipo=3,'Cancelacion',tipo))) as tipo, 
				date_recieved, 
				date_created, 
				IF(a.gds=5,NULL,TIMEDIFF(date_created,date_recieved)) as Tiempo,
				PNR, if(`status`=1,'Emitido',if(`status`=0,'Regresado',`status`)) as `status`, 
				Nombre 
			FROM 
				trfMT_emisiones a 
			LEFT JOIN 
				trf_gds b ON a.GDS=b.trf_gds_id 
			LEFT JOIN 
				Asesores c ON a.asesor=c.id
			WHERE
				CAST(a.date_created as DATE) BETWEEN '$inicio' AND '$fin'
			ORDER BY 
				date_created DESC ";
	...
	for($i=0;$i<$num;$i++){
		...
		$data[mysql_result($result,$i,'id')]['Tipo']=mysql_result($result, $i, 'tipo');
		$data[mysql_result($result,$i,'id')]['Fecha Recepción']=mysql_result($result, $i, 'date_recieved');
		$data[mysql_result($result,$i,'id')]['Fecha Emisión']=mysql_result($result, $i, 'date_created');
		$data[mysql_result($result,$i,'id')]['Tiempo Transcurrido']=mysql_result($result, $i, 'tiempo');
		$data[mysql_result($result,$i,'id')]['PNR']=mysql_result($result, $i, 'PNR');
		$data[mysql_result($result,$i,'id')]['Status']=mysql_result($result, $i, 'status');
	...
	$('#result').tablesorter({
        ...
        widgetOptions: {
           ...
           uitheme: 'jui',
            ...
            output_saveFileName  : 'emisiones-<?php echo "$inicio a $fin";?>.csv',
...
<table class='t2' style='width:600px; margin:auto'><form action="emisiones.php" method="post" enctype="multipart/form-data">
	<tr class='title'>
		<th colspan=10>Reporte de Emisiones <?php if(isset($_POST['submit'])){echo " ($inicio a $fin)";} ?></th>
	</tr>
	<tr class='title'>
