...
//List Actividades
$query="SELECT * FROM bo_actividades WHERE area=1 ORDER BY actividad";
...
$(function() {
    ...
    function sendRequest(variables,hora,loc,comment){
        var urlsend= "/json/formularios/us_comments.php?"+variables;
        var xmlhttp;
        var text;
    ...
    function submitForm(){
        ...
        var submit_com=$('#rango').attr('required');
        var field_loc=$('#localizador');
        var field_com=$('#rango');
        var field_val=true;
       if(submit_loc=='required' || field_loc.val()!=""){
            field_val= field_val && checkLength( field_loc, "Localizadior", 7, 9 );
            field_val= field_val && checkRegexp( field_loc, /[0-9][0-9][0-9][0-9][0-9][0-9][0-9]/, "El formato solicitado para localizadores es 1234567-12" );
        }
        ...
        if(field_val){
            ...
            var casos="";
            var casotemp;
            var temp=field_com.val();
                casotemp=temp.substr(0,13)+" ... "+temp.substr(temp.length-10,10);
                var casosok=temp.replace(/\s/g, " ");
                casos=casosok;

            var urlok="comment="+casos+"&localizador="+field_loc.val();
            sendRequest(urlok,timenow,field_loc.val(),casos);
        }
...
$query="SELECT localizador, comentario, CONCAT(DAY(date_created),'-',MONTH(date_created),'-',YEAR(date_created)) as fecha FROM us_comentarios WHERE date_created>='".date('Y-m-d')."' AND user=".$_SESSION['id'];
$result=mysql_query($query);
$num=mysql_numrows($result);
$i=0;
while($i<$num){
    $reg_hora[$i]=mysql_result($result,$i,'fecha');
    $reg_loc[$i]=mysql_result($result,$i,'localizador');
    $reg_comment[$i]=mysql_result($result,$i,'comentario');
$i++;
}
...
<table width='100%' class='t2'>
        <tr class='title'>
            <td colspan=100>Formulario UpSell</td>
        </tr>
    </table>
    <br>
...
<div id="wrapper">
  <div id="content">
    ...
    <tr class='title' id='tabLoc1'>
            <th>Localizador (#######)</th>
            <td class='pair'><input type="text" name='localizador' id='localizador' value='' required></td>
        </tr>
        <tr class='title'  id='tabComment1'>
        <th>Comentarios</th>
        <td class='pair'><textarea name='rango' id='rango' value='' rows='10' required></textarea></td>
    </tr>
    <tr class='total'>
  ...
  <div id="sidebar">
   ...
   <table width='100%' id='hor-minimalist-a' class='tablesorter' >
        ...
        <?php
            unset($key,$iden);
            foreach($reg_loc as $key => $localizador){
                echo "<tr>\n";
                echo "<td width='25%'>$reg_hora[$key]</td>\n";
                echo "<td width='25%'>$localizador</td>\n";
                echo "<td width='25%'>$reg_comment[$key]</td>\n";
                echo "</tr>\n";
            }
