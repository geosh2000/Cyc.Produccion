...
initSettings::start(true,'asesor_formulario_obmt');
initSettings::printTitle('Tipificación OB-MT');
...
$query="SELECT * FROM OBMT_opts WHERE activo=1 ORDER BY actividad, nivel, opcion";
...
$(function(){
    ...
    function validateFields(){
    	...
    	$('select').each(function(){
    	 	...
    	 	if($(this).val()=='' && !$(this).closest('.containers').is(':hidden')){
    	 		validfields = false;
			    $(this).addClass('error');
    	 	}
    	 });
    	 ...
    	 $('input:text').each(function(){
    	 	$(this).removeClass('error');
    	 	parameters[$(this).attr('id')] = removeDiacritics($(this).val()).toUpperCase();

    	 	var regextxt = (typeof $(this).attr('regex') === 'undefined') ? false : true;
    	 	var required = $(this).attr('req');
    	 	var texto=$(this).val();

    	 	if(required==1 && texto==""  && !$(this).closest('.containers').is(':hidden')){
    	 		validfields = false;
	       		$(this).addClass('error');
    	 	}

    	 	if(texto!=""  && !$(this).closest('.containers').is(':hidden')){
    	 		if(regextxt){
	...
	$('.camposel').change(function(){
		var valor=$(this).val();
		var trig = $('option:selected' , this).attr('trig');
		$('.opts').each(function(){
			var parent = $(this).attr('parent');
			if(parent!=0){
				parentflag=false;
		...
		$('.containers').each(function(){
			...
			if(actividad!=0){
				actflag=false;
				$('.camposel').each(function(){
					if($('option:selected', this).attr('trig')==actividad){
						actflag=true;
					}
				});
...
<div class='formulario'>
    <div class='titulo'>
        <p style='padding-top: 13px; color: white;'>OB MT - Funciones</p>
    ...
    <?php
    	foreach($opts as $actividad => $data){
    		foreach($data as $nivel => $info){
				echo "<div id='contain-level".$nivel."' class='campo containers actividad_$actividad' level='$nivel' actividad='$actividad'>
			        	<div class='name'>
