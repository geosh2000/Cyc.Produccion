...
initSettings::printTitle('Reporte de Tipificación OB-MT');
...
if(isset($_POST['from'])){
	...
	$query="SELECT nivel, titulo FROM OBMT_opts GROUP BY nivel ORDER BY nivel";
	...
	$query="SELECT a.id, NombreAsesor(asesor,1) as Asesor, date_created,
				...
				IF(j.opcion IS NULL, level9, j.opcion) as level9  FROM OBMT_tipificacion a
			LEFT JOIN OBMT_opts b ON a.level1=b.id
			LEFT JOIN OBMT_opts c ON a.level2=c.id
			LEFT JOIN OBMT_opts d ON a.level3=d.id
			LEFT JOIN OBMT_opts e ON a.level4=e.id
			LEFT JOIN OBMT_opts f ON a.level5=f.id
			LEFT JOIN OBMT_opts g ON a.level6=g.id
			LEFT JOIN OBMT_opts h ON a.level7=h.id
			LEFT JOIN OBMT_opts i ON a.level8=i.id
			LEFT JOIN OBMT_opts j ON a.level9=j.id
			WHERE
