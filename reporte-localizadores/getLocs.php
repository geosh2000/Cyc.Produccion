...
switch ($tipo) {
  case 'locs':
    $query="SELECT
            locs_id, Fecha, Hora, nombreAsesor(asesor,2) as Asesor, FindSupDay(asesor,Fecha) as Supervisor, Localizador, Servicios, Afiliado, VentaMXN as Venta, OtrosIngresosMXN as Otros_Ingresos, EgresosMXN as Egresos, VentaMXN+OtrosIngresosMXN+EgresosMXN as Monto_Total
          FROM
            (SELECT dep, a.* FROM
              (SELECT b.Fecha, id, dep FROM Fechas b JOIN Asesores a LEFT JOIN dep_asesores c ON a.id=c.asesor AND b.Fecha=c.Fecha WHERE b.Fecha BETWEEN '$inicio' AND '$fin' HAVING dep=$dep) ases
            RIGHT JOIN
