...
<table id='searchtable' class='t2' style='width:800px; margin: auto'>
	...
	<tr class='title'>
		<td class='pair'><select name='skill' required><option value=''></option>
			<?php
				$query="SELECT * FROM PCRCs WHERE parent_group=1 ORDER BY Departamento";
				$result=mysql_query($query);
				$num=mysql_numrows($result);
				for($i=0;$i<$num;$i++){
					if($skill==mysql_result($result, $i, 'skills')){
						$seltmp="selected";
					}else{
						$seltmp="";
					}
					
					echo "<option value=\"".mysql_result($result, $i, 'skills')."\" $seltmp>".mysql_result($result, $i, 'Departamento')."</option>\n\t";
				}
			?>
		</select></td>
		...
		LEFT JOIN
			(
				...
				ORDER BY
					`jornada start`) pya
			ON 
				a.Hora_time BETWEEN `jornada start` AND IF(`jornada end`<'08:00:00',ADDTIME(`jornada end`,'24:00:00'),`jornada end`)  OR
				a.Hora_time BETWEEN `extra1 start` AND IF(`extra1 end`<'08:00:00',ADDTIME(`extra1 end`,'24:00:00'),`extra1 end`)
			WHERE
			 `N Corto` IS NOT NULL";
$result=mysql_query($query);
if(mysql_error()){
...
for($i=0;$i<$num;$i++){
	for($x=0;$x<$numfields;$x++){
		if((mysql_field_name($result,$x)=='Login' || mysql_field_name($result,$x)=='Logout') && mysql_result($result,$i,$x)!=NULL){
			...
			$pya[mysql_result($result,$i,'N Corto')][mysql_field_name($result,$x)]=$time_tmp->format('H:i:s');
		}else{
			$pya[mysql_result($result,$i,'N Corto')][mysql_field_name($result,$x)]=mysql_result($result,$i,$x);
		}
	}
	//Display
...
if(date('Y-m-d',strtotime($date))<date('Y-m-d')){
	$query="SELECT *
				...
				LEFT JOIN
					...
					ON 
						a.Hora_time BETWEEN `Hora` AND Hora_out
					WHERE
						`N Corto` IS NOT NULL
				 ";
}else{
...
$num=mysql_numrows($result);
for($i=0;$i<$num;$i++){
	$time_tmp = new DateTime($date.' '.mysql_result($result,$i,'Hora_time').' America/Mexico_city');
	$time_tmp -> setTimezone($cun_time);
	...
	$sesion[mysql_result($result,$i,'N Corto')][$indextmp]=mysql_result($result,$i,'Hora')." - ".mysql_result($result,$i,'Hora_out');
}
