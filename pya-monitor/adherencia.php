...
<table id='searchtable' class='t2' style='width:800px; margin: auto'>
	<tr class='title'><form action='<?php $_SERVER['PHP_SELF']; ?>' method='POST' id='seleccion'>
		...
		<th>Departamento</th>
		<th>Monitor de PyA (<?php echo date('d M Y', strtotime($dateok)); ?>)</th>
	</tr>
	<tr class='title'>
		<td class='pair'><select name='skill' required><option value=''></option>
			<?php
				$query="SELECT skills, Departamento FROM PCRCs WHERE parent_group=1 ORDER BY Departamento";
				if($result=Queries::query($query)){
					while($fila=$result->fetch_assoc()){
						if($skill==$fila['skills']){
							$seltmp="selected";
						}else{
							$seltmp="";
						}
						
						echo "<option value=\"".$fila['skills']."\" $seltmp>".$fila['Departamento']."</option>\n\t";
					}
				}
				
			?>
		</select></td>
		<th>Ultima Actualizacion: <?php echo date('d-m-Y  H:i:s', strtotime($lastupdate)); ?><l id='demo'></l></th>
	</tr></form>
</table>
		...
		LEFT JOIN
			(
				...
				ORDER BY
					`jornada start`) pya
			ON 
				ADDTIME(a.Hora_time,'00:01:00') BETWEEN `jornada start` AND IF(`jornada end`<'06:00:00',ADDTIME(`jornada end`,'24:00:00'),`jornada end`)  OR
				ADDTIME(a.Hora_time,'00:01:00') BETWEEN `extra1 start` AND IF(`extra1 end`<'06:00:00',ADDTIME(`extra1 end`,'24:00:00'),`extra1 end`)
			WHERE
			 `N Corto` IS NOT NULL";
if($result=Queries::query($query)){
	...
	while($fila=$result->fetch_array(MYSQLI_BOTH)){
		for($x=0;$x<$num_fields;$x++){
			if(($fields[$x]->name=='Login' || $fields[$x]->name=='Logout' || $fields[$x]->name=='jornada start' || $fields[$x]->name=='jornada end') && $fila[$x]!=NULL){
				$time_tmp = new DateTime($date.' '.$fila[$x].' America/Mexico_city');
				$time_tmp -> setTimezone($cun_time);
				$pya[$fila['N Corto']][$fields[$x]->name]=$time_tmp->format('H:i:s');
			}else{
				$pya[$fila['N Corto']][$fields[$x]->name]=$fila[$x];
			}
		}
		//Display
		if($fila['Ausentismo']==NULL){
			$block[$fila['Hora_int']][]=$fila['N Corto'];
		}
	}
}else{
	echo "ERROR! -> ";
...
foreach($pya as $asesor => $info2){
		$pya[$asesor]['validado']="OK";
		...
		if($info2['Ausentismo']==NULL && date('Y-m-d',strtotime($date))<=date('Y-m-d')){
			if($info2['Login']==NULL){
				if(date('H:i:s')>date('H:i:s', strtotime($info2['jornada start'].' +1 minutes'))){
					if(date('H:i:s')>date('H:i:s', strtotime($info2['jornada start'].' +10 minutes'))){
						$pya[$asesor]['Retardo_tipo']="B";
					}else{
						$pya[$asesor]['Retardo_tipo']="A";
					}
				}	
			}else{
			...
			if(isset($pya[$asesor]['Retardo_tipo'])){
				//Excepcion
				if($info2['Excepcion']!=NULL){
					$excep="<br>".$info2['Excepcion'];
					$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['Tit']="Excepcion Aplicada por:<br><a style='color: red'>".$info2['excep_aplicado']."</a><br>".$info2['excep_f_aplicado']."<br>caso: ".$info2['excep_caso']."<br>Nota: ".$info2['excep_nota'];
					if($info2['Excepcion']!="Retardo Notificado"){$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['Excep']=1;}	
				}else{
					$excep="";
				...
				//Login
				if($info2['Login']!=NULL){
					$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['Login']=1;	
				}
				
				$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['Ret']="$asesor<br>(".$info2['Departamento'].")<br>js: ".$info2['jornada start']."<br>li: ".$info2['Login'].$excep;
				$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['ncorto']=$asesor;
				$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['fecha']=$date;
				$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['asesorid']=$info2['asesorid'];
				$retardos[$pya[$asesor]['Retardo_tipo']][$asesor]['horaid']=$info2['horarioid'];
			}
		}
}
unset($asesor,$info2);
...
if(date('Y-m-d',strtotime($date))<date('Y-m-d')){
	$query="SELECT *
				...
				LEFT JOIN
					(
						SELECT 
							Hora_in as Hora, Hora_out, `N Corto` 
						FROM 
							t_Sesiones a 
						LEFT JOIN 
					...
					ON 
						ADDTIME(a.Hora_time,'00:29:59') BETWEEN `Hora` AND Hora_out
					WHERE
						`N Corto` IS NOT NULL
				 ";
}else{
...
if($result=Queries::query($query)){
	while($fila=$result->fetch_assoc()){
		$time_tmp = new DateTime($date.' '.$fila['Hora_time'].' America/Mexico_city');
		$time_tmp -> setTimezone($cun_time);
		if(date('i',strtotime($time_tmp->format('H:i:s')))>=30){
			$indextmp=intval(date('H',strtotime($time_tmp->format('H:i:s'))))*2+1;
		}else{
			$indextmp=intval(date('H',strtotime($time_tmp->format('H:i:s'))))*2;
		}
		
		$sesion[$fila['N Corto']][$indextmp]=$fila['Hora']." - ".$fila['Hora_out'];
		$hora[$indextmp]=$time_tmp->format('H:i:s');
	}
}
?>
<style>
...
<br>
<table id='retardostable' class='t2' style='margin: auto; text-align: center; '>
		...
		<?php
			for($i=25;$i>=-1;$i--){
				...
				echo "<tr><td class='title' style='font-size:10px'>$head</td><td style='text-align: center; padding: 11 0 5 0; width: 1200'>\n";
				for($x=0;$x<48;$x++){
					if($i==-1){
						echo "<div class='drop-shadow curved v2 block blue'>".($x/2)."</div>\n\t";	
					}else{
						if(isset($block[$x][$i])){
							//check session
							if(isset($sesion[$block[$x][$i]][$x])){
								if(date('H:i:s',strtotime($pya[$block[$x][$i]]['Login']))>date('H:i:s',strtotime($hora[$x].' +1 minutes'))){
									$classtmp="orange";
								}else{
									$classtmp="green";
								}
								$titletmp=$block[$x][$i]."<br>h: ".$pya[$block[$x][$i]]['jornada start']." - ".$pya[$block[$x][$i]]['jornada end']."<br>ses: ".$pya[$block[$x][$i]]['Login']." - ".$pya[$block[$x][$i]]['Logout'];
							}else{
								if(date('i')>=30){
									$indextmp=intval(date('H'))*2+1;
								}else{
									$indextmp=intval(date('H'))*2;
								}

								if($x<$indextmp || date('Y-m-d',strtotime($date))<date('Y-m-d')){
									/*	
									if($pya[$block[$x][$i]]['Login']==NULL){
										$classtmp='gray';	
									}else{
										$classtmp='orange';	
									}
									*/
									...
									$titletmp=$block[$x][$i]."<br>h: ".$pya[$block[$x][$i]]['jornada start']." - ".$pya[$block[$x][$i]]['jornada end']."<br>ses: ".$pya[$block[$x][$i]]['Login']." - ".$pya[$block[$x][$i]]['Logout']."<br>excep:".$pya[$block[$x][$i]]['Excepcion'];
								}else{
									$classtmp='gray';
									$titletmp=$block[$x][$i]."<br>h: ".$pya[$block[$x][$i]]['jornada start']." - ".$pya[$block[$x][$i]]['jornada end']."<br>excep:".$pya[$block[$x][$i]]['Excepcion'];
								}
							}
							
							echo "<div class='drop-shadow curved v2 block $classtmp' title='$titletmp'></div>\n\t";	
							
						}else{
							echo "<div class='drop-shadow curved v2 block white'></div>\n\t";	
						}
					}
				}
				echo "</td></tr>\n";
