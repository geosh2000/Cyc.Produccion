...
$query="SELECT
        ...
        FROM
        (SELECT a.id, c.Fecha, Nombre, IF(getDepartamento(a.id, c.Fecha) IS NULL, getLastDep(a.id), getDepartamento(a.id, c.Fecha)) as dep FROM Asesores a JOIN (SELECT Fecha FROM Fechas WHERE Fecha BETWEEN '$start' AND '$end') c WHERE (Egreso >= '$end' AND Egreso IS NOT NULL) HAVING dep IN ($skill) ORDER BY Nombre) a
        LEFT JOIN
        (SELECT
