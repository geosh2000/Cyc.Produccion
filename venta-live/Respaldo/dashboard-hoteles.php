...
$query="DROP TEMPORARY TABLE IF EXISTS dash_td; 
        ...
        INSERT INTO locs_shown (SELECT * FROM (SELECT * FROM d_hoteles WHERE Fecha BETWEEN ADDDATE(CURDATE(),-1) AND CURDATE()) a ) ON DUPLICATE KEY UPDATE Venta=a.Venta; 
        ...
        CREATE TEMPORARY TABLE dash_td 
                SELECT 
                        Fecha, Hora_int, Hora_pretty, 
                CASE 
                                WHEN chanId IN (295,355) THEN 'Outlet' $canales END as OAfiliado, 
                COUNT(DISTINCT VentaHoy) as Locs, 
                SUM(MontoDia) as Monto, 
                SUM(VentaMXN+OtrosIngresosMXN) as SoloVenta, 
                RN 
                FROM 
                        HoraGroup_Table15 a 
                RIGHT JOIN 
...
$(function(){
  ...
  setInterval(function(){
    $('#timer').text(time--);
    if(time==0){
      location = '/venta-live/dashboard-hoteles.php?det='+det;
    }
  },1000);
});
</script>
