...
if($result=$connectdb->query($query)){
  ...
  $var['ly']['inicio']=date('Y-m-d',strtotime('-365 days'));
  $var['ly']['fin']=date('Y-m-d',strtotime('-364 days'));
  $var['td']['inicio']=date('Y-m-d',strtotime('-1 days'));
  $var['td']['fin']=date('Y-m-d');
...
$query="DROP TEMPORARY TABLE IF EXISTS dash_td;
    ...
    CREATE TEMPORARY TABLE dash_td (SELECT Fecha, Hora_int, Hora_pretty, 
        ...
        COUNT(DISTINCT VentaHoy) as Locs, SUM(MontoDia)*160.42 as Monto, SUM(VentaMXN+OtrosIngresosMXN) as SoloVenta 
        FROM 
          HoraGroup_Table15 a 
        RIGHT JOIN 
          (SELECT 
...
$(function(){
  ...
  setInterval(function(){
    $('#timer').text(time--);
    if(time==0){
      location = '/venta-live/dashboard-co.php?det='+det;
    }
  },1000);
});
</script>
