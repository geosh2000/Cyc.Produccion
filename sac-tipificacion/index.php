...
initSettings::start(true,'asesor_tipificacion_sac');
initSettings::printTitle('Tipificación SAC IN');
...
$(function(){
    ...
    function search_motivo(variables){
      ...
      $.ajax({
        ...
        error: function(){
          showNoty('error', 'Error de conexión, intenta nuevamente', 4000);
        }
      });
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          if(status[1]=='OK'){
              ...
              $('.input').val('');
              start_fields();
              showNoty('success', notif_msg[1],4000);
          }else{
              showNoty('error', notif_msg[1],4000);
          }
...
$query="SELECT a.Fecha, a.asesor, a.Calls as llamadas, b.Registros  FROM
            ...
            ON a.Fecha=b.Fecha AND a.asesor=b.asesor LIMIT 1";
    ...
    <div id='contain-mg' class='campo'>
        ...
        <div class='opcion'>
            ...
            <?php
                ...
                if($result=$connectdb->query($query)){
                  while($fila=$result->fetch_assoc()){
                    echo "<option value='".$fila['id']."'>".$fila['Motivo_General']."</option>";
                  }
                }
            ?></select>*</p>
        </div>
    ...
    <div id='contain-me' class='campo'>
        ...
        <div class='opcion'>
            <p class='seleccion'><select id='f_me' class='input'><option value=''>Selecciona...</option><?php
                ...
                if($result=$connectdb->query($query)){
                  while($fila=$result->fetch_assoc()){
                    echo "<option class='id_me me".$fila['motivo_general']."' value='".$fila['id']."'>".utf8_encode($fila['Motivo_Especifico'])."</option>";
                  }
                }
    ...
    <div id='contain-detalle' class='campo'>
        ...
        <div class='opcion'>
            <p class='seleccion'><select id='f_detalle' class='input'><option value=''>Selecciona...</option><?php
                ...
                if($result=$connectdb->query($query)){
                  while($fila=$result->fetch_assoc()){
                    echo "<option class='id_detalle detalle".$fila['motivo_especifico']."' value='".$fila['id']."' txt='".$fila['Texto']."'>".utf8_encode($fila['Detalle'])."</option>\n\t";
                  }
                }
            ?></select>*</p>
        </div>
