...
$(function() {
    ...
    function sendRequest(variables,hora,act,fecha,em){
        ...
        var urlsend= "/json/formularios/bo_mailing.php?"+variables;
        var xmlhttp;
        var text;
    ...
    function submitForm(){
        ...
        if(field_val){
            ...
            var casotemp;
            if(field_rango.val()!=""){
                var temp=field_rango.val();
                casotemp=temp.substr(0,13)+" ... "+temp.substr(temp.length-10,10);
                var casosok=temp.replace(/\s/g, " ");
                casos=casosok;
            }else{casotemp=field_em.val();}
            var urlok="actividad="+field_act.val()+"&rango="+casos+"&em="+field_em.val()+"&fecha_asignacion="+field_fa.val()+"&hr_asignacion="+field_ha.val()+"&min_asignacion="+field_ma.val();
            sendRequest(urlok,timenow,actividad_name,field_fa.val(),casotemp);
        }
...
$query="SELECT confirming_id, b.actividad, em, date_created, fecha_recepcion FROM bo_mailing a, bo_actividades b WHERE a.actividad=b.bo_act_id AND date_created>='".date('Y-m-d')."' AND user=".$_SESSION['id'];
$result=mysql_query($query);
$num=mysql_numrows($result);
$i=0;
while($i<$num){
