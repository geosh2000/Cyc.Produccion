...
$query="SELECT id, `N Corto`, getDepartamento(id,'$fecha') as dep, getPuesto(id,'$fecha') as Super FROM Asesores WHERE Egreso>'$fecha' AND Ingreso<='$fecha' HAVING dep NOT IN (29) AND Super IN (11,18,19) ORDER BY `N Corto`";
...
$query="SELECT a.id, Asesor, Supervisor, b.id as idsuper, color FROM
	(SELECT
		a.id, `N Corto` as Asesor, IF(FindSupDay(a.id,'$fecha') IS NULL,'0nosup',FindSupDay(a.id,'$fecha')) as Supervisor, getDepartamento(a.id,'$fecha') as Dep
	FROM
		Asesores a
	WHERE
		Ingreso<='$fecha' AND Egreso>='$fecha'
	HAVING
		Dep NOT IN (12,1,29,30,31)
        ) a
	LEFT JOIN
		Asesores b
	ON
...
if($result=Queries::query($query)){
	...
	while($fila=$result->fetch_assoc()){
	    ...
	    if($tmp!=$fila['Supervisor']){
			...
			if(!in_array($fila['Supervisor'],$sup)){
				$sup[]=$fila['Supervisor'];
				$idsup[]=$fila['idsuper'];
			}
