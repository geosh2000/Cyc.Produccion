...
initSettings::start(true,'asesor_formulario_ag');
initSettings::printTitle('Agencias BO - Tipificación');
...
$(function(){
  ...
  flag=true;
    ...
    function start_fields(){
        $('#f_caso, #f_localizador, #f_tipo, #date-in, #dateout').val('');
        $('#contain-caso, #contain-localizador, #contain-tipo, #contain-datein, #contain-dateout').attr('req',1);
    ...
    function validate(){
        ...
        if(flag_caso && flag_localizador && flag_datein &&  flag_tipo){flag=true;}else{flag=false; $('#submit_form').prop('disabled',false);}
    ...
    $('#submit_form').click(function(){
        ...
        validate();
        if(flag){
            //var variables="asesor=<?php echo $asesor; ?>&area=<?php echo $area; ?>&caso=" + $('#f_caso').val() + "&localizador=" + $('#f_localizador').val() + "&datec=" +  $('#date-in').val() + "&tipo=" +  $('#f_tipo').val();
            variables = {asesor: <?php echo $asesor; ?>, area:<?php echo $area; ?>, caso: $('#f_caso').val(), localizador: $('#f_localizador').val(), datec: $('#date-in').val(), tipo: $('#f_tipo').val()};
            //alert(variables);
...
<div class='formulario'>
    <div class='titulo'>
        <p style='padding-top: 13px; color: white;'>Agencias BackOffice - Mejora Continua</p>
    ...
    <div id='contain-tipo' class='campo'>
        ...
        <div class='opcion'>
            ...
            <?php
                $query="SELECT * FROM ag_bo_status WHERE area=$area ORDER BY status";
