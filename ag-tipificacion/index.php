...
initSettings::printTitle('Tipificación Agencias');
...
$(function(){
    ...
    function start_fields(){
        ...
        $('#contain-agencia, #contain-localizador, #contain-canal, #contain-motivo, #contain-submit, #contain-nombre').show().attr('req',1);
    ...
    function validate(){
        ...
        if(flag_agencia && flag_nombre && flag_canal && flag_localidad && flag_motivo && flag_tipo && flag_soporte){flag=true;}else{flag=false;}
    ...
    /*$('#f_agencia, #f_localidad, #f_nombre').keyup(function(){
        var name=$(this).val();
    ...
    $('#submit_form').click(function(){
        validate();
        if(flag){
            var variables="asesor=<?php echo $asesor; ?>&agencia=" + $('#f_agencia').val() + "&nombre=" + $('#f_nombre').val() + "&canal=" + $('#f_canal').val() + "&localidad=" + $('#f_localidad').val() + "&motivo=" + $('#f_motivo').val() + "&tipo=" + $('#f_tipo').val() + "&soporte=" + $('#f_soporte').val() + "&localizador=" + $('#f_localizador').val();
    ...
    <div id='contain-tipo' class='campo'>
        ...
        <div class='opcion'>
            <p class='seleccion'><select id='f_tipo' class='input'><option value=''>Selecciona...</option><?php
                ...
                if($result=Queries::query($query)){
			    	while ($fila=$result->fetch_Assoc()) {
			        	echo"<option class='id_tipo tipo".$fila['motivo']."' value='".$fila['id']."'>".$fila['tipo']."</option>";
                	}
                }
            ?></select>*</p>
        </div>
        ...
        <div class='opcion'>
            <p class='seleccion'><select id='f_soporte' class='input'><option value=''>Selecciona...</option><?php
                $query="SELECT * FROM ag_soporte ORDER BY tipo_soporte";
                 if($result=Queries::query($query)){
			    	while ($fila=$result->fetch_Assoc()) {
			        	echo "<option class='id_soporte soporte".$fila['soporte']."' value='".$fila['id']."'>".$fila['tipo_soporte']."</option>";
					}
                }
