...
foreach($data as $key1 => $row){
	...
	if($q_var!=""){
		$query="SELECT * FROM $db WHERE Fecha='$fechaok' AND afiliado='".$data[$key1][2]."' AND asesor='".$data[$key1][1]."'";
		$num=mysql_numrows(mysql_query($query));
		
		if($num>0){
			$md_id=mysql_result(mysql_query($query),0,'serv_id');
			$query="UPDATE $db SET $q_ins, Fecha='$fechaok' WHERE serv_id=$md_id"; 
			echo "$query<br>";
			mysql_query($query);
		}else{ 
