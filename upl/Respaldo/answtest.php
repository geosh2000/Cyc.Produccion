...
$err_count=0;
$tipo='ans';
$okfilename="ans_".$_GET['file'];

if(intval($_GET['file'])+1<10){$next=(intval($_GET['file'])+1);$estefile=(intval($_GET['file']));}else{$next=(intval($_GET['file'])+1);$estefile=$_GET['file']; }
$endoffiles='26';
if(isset($_GET['reg'])){$reg=$_GET['reg'];}else{$reg=0;}

if($_GET['file']>$endoffiles){ echo "End of files!!"; exit;}
...
//Read CSV
function printTable(){
	global $data, $regs, $tipo, $okfilename;
	$fila = 1;
	if (($gestor = fopen("../uploads/$okfilename.csv", "r")) !== FALSE) {
	    echo "\n<table class='t2' style='width:100%'>\n";
	    while (($datos = fgetcsv($gestor, 1000, ";")) !== FALSE) {
	    	$regs=$fila;
            $dataok=str_replace("\"","",$datos[0]);
            //$datos=explode(";",$dataok);
            //print_r($datos);
	    	$data[]=$datos;
            //echo $dataok;
	    	if($fila==1){
...
location.replace(\"answtest.php?file=$estefile&reg=14800\");
</script>";
    }
    if($key>$reg){
	if($key!=0){
	...
	$x=0;
		...
		if($data[$key][1]!='*'){
            ...
            /*$query_check="SELECT `ac_id` FROM `$db` WHERE AsteriskID=".$data[$key][$astid]." AND Fecha='$fecha_inicio'";
...
unlink("../uploads/$okfilename.csv");
if(count($data)==0){
    echo "ERROR! on file $okfilename";
}else{
    $query="INSERT INTO temp_regsup (file,regs,errores) VALUES('$okfilename','".count($data)."','$err_count')";
    mysql_query($query);
    echo "Done!...<br>$err_count errors.";
    echo "<script type=\"text/javascript\">
location.replace(\"answtest.php?file=$next\");
</script>";
}
