...
foreach($data as $key => $printinfo){
	...
	//get userid
		...
		$result=mysql_query($query);
		if(mysql_result($result,0,'id')!=NULL){$userid[$key]=mysql_result($result,0,'id');}else{
            $userid[$key]= $printinfo[8];
		}
	}else{$userid[$key]="";}
	$x=0;
		...
		foreach($printinfo as $key2 => $info){
				...
				if($key2==1){
					...
					$fecha_inicio='2016-'.substr($printinfo[$key2-1],0,2).'-'.substr($printinfo[$key2-1],3,2);
					$q_var=$col[$x].",".$col[$x+1];
