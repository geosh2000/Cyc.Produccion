...
$query="SELECT a.id, date_created, fecha_recepcion, b.status, em, localizador FROM bo_tipificacion a LEFT JOIN bo_status b ON a.status=b.id WHERE CAST(date_created as DATE)=CURDATE() AND asesor=$asesor AND a.area=$area ORDER BY a.id DESC";
...
$query="SELECT COUNT(*) as casos FROM bo_tipificacion a WHERE asesor=$asesor AND status!=8 AND CAST(a.date_created as DATE)=CURDATE()";
if($result=$connectdb->query($query)){
	while($fila=$result->fetch_assoc()){
		$casos=$fila['casos'];
	}
