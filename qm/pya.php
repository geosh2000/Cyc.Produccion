...
$query="SELECT 
			a.id as asesorid, b.id as horarioid, `N Corto`, Departamento, IF(LogAsesor$function(b.Fecha,b.asesor,'in')>'21:00:00','00:00:00',LogAsesor$function(b.Fecha,b.asesor,'in')) as Login, 
		...
		WHERE
			Activo=1 AND Fecha='$date' AND `jornada start`!=`jornada end`
		ORDER BY
			Departamento, Nombre";
		...
		if($info2['Ausentismo']==NULL && date('Y-m-d',strtotime($date))<=date('Y-m-d')){
			if($info2['Login']==NULL){
				if(date('H:i:s')>=date('H:i:s', strtotime($info2['jornada start'].' +1 minutes'))){
					if(date('H:i:s')>date('H:i:s', strtotime($info2['jornada start'].' +13 minutes'))){
			...
			foreach($pya as $departamento => $info){
				...
				foreach($info as $asesor => $info2){
					if(date('H:i', strtotime($info2['jornada start']))==$x){
						...
						."$asesor<br>($departamento)<br>$showlog</div>";
					}
