...
function p_rtcalls(){
	...
	foreach($rtcalls as $sknum => $info){
	    ...
	    //echo "Skill: $sknum || Callsin-call: ".count($info['in-call'])." || AHT: ".number_format(array_sum($info['duration'])/count($info['duration']),2)." || WaitingCalls: ".count($info['Waiting'])." || LongestCallW: ".max($info['waittime'])." || Agents: ".count($info['agents'])." || Avail: ".array_sum($info['agents'])." || Paused: ".array_sum($info['OnPause'])."<br>";
	    echo "-callsin$sknum- ".count($info['InCalls'])." -callsin$sknum- "
	    	."-callsout$sknum- ".count($info['OutCalls'])." -callsout$sknum- "
	        ." -aht$sknum- ".gmdate('H:i:s',(array_sum($info['in-dur'])+array_sum($info['out-dur']))/(count($info['in-dur'])+count($info['out-dur'])))." -aht$sknum- "
	        ." -ahtseg$sknum- ".(array_sum($info['in-dur'])+array_sum($info['out-dur']))/(count($info['in-dur'])+count($info['out-dur']))." -ahtseg$sknum- "
	        ." -waitingcalls$sknum- ".count($info['Waiting'])." -waitingcalls$sknum- "
	        ." -longestcallw$sknum- ".gmdate('H:i:s',intval("0".max($info['waittime'])))." -longestcallw$sknum- "
	        ." -longestcalli$sknum- ".gmdate('H:i:s',intval("0".max($info['in-dur'])))." -longestcalli$sknum- "
	        ." -agents$sknum- ".count($info['agents'])." -agents$sknum- "
	        ." -avail$sknum- ".array_sum($info['agents'])." -avail$sknum- "
	        ." -paused$sknum- ".intval("0".array_sum($info['OnPause']))." -paused$sknum- <br> ";
	}
}
...
function p_asesor($skill_get){
	global $asesor, $skill, $getskill, $ext_est;
	$i=0;
	if(count($asesor)>0){
		foreach($asesor as $agent => $data){
			...
			if(strpos($agent,"(")==0){
				...
				$tmpagent_noext=substr($agent,0,strpos($agent,"(")).$tmpEst;
			...
			if($flag){
				...
				echo "-asesor$i- ".$asesor_name." -asesor$i- \n"
					."-asesornoext$i- $tmpagent_noext -asesornoext$i- \n"
					."-status$i- $tmpstatus -status$i- \n"
					."-savail$i- ".$data['Avail']." -savail$i- \n"
					."-availseg$i- ".$data['Availseg']." -availseg$i- \n"
					."-caller$i- ".$data['Call']['Caller']." -caller$i- \n"
					."-queue$i- ".str_replace("Servicio a Cliente", "SAC", $data['Call']['Queue'])." -queue$i- \n"
					."-calldur$i- ".$data['Call']['Duration']." -calldur$i- \n"
					."-callseg$i- ".$data['Call']['seg']." -callseg$i- \n"
					."-pausem$i- ".$data['Pause']['Motivo']." -pausem$i- \n"
					."-pauseseg$i- ".$data['Pause']['seg']." -pauseseg$i- \n"
					."-pausedur$i- ".$data['Pause']['tiempo']." -pausedur$i- \n"
					."-processed$i- ".$processed." -processed$i- \n";
