...
$query="SELECT Fecha, Hora_int, Hora_pretty, b.Canal as Afiliado, COUNT(DISTINCT NewLoc) as Locs, SUM(VentaMXN+EgresosMXN+OtrosIngresosMXN) as Monto 
        FROM 
          HoraGroup_Table15 a 
        RIGHT JOIN 
          (SELECT 
            a.*, IF(Venta!=0,Localizador,NULL) as NewLoc, AfiliadoOK as Canal 
          FROM 
            t_Locs a 
          LEFT JOIN chanIds b ON a.chanId=b.id 
          WHERE 
            Fecha BETWEEN '2016-05-12' AND '2016-05-15' 
            AND Canal='MP'
            AND Pais='MX') b 
...
$query="SELECT Fecha, Hora_int, Hora_pretty, b.Canal as Afiliado, COUNT(DISTINCT NewLoc) as Locs, SUM(VentaMXN+EgresosMXN+OtrosIngresosMXN) as Monto 
        ...
        RIGHT JOIN 
          (SELECT 
				...
				LEFT JOIN chanIds b ON a.chanId=b.id 
				WHERE 
					Fecha BETWEEN '2017-05-11' AND '2017-05-14' 
					AND Canal='MP'
					AND Pais='MX') b 
          ON b.Hora BETWEEN a.Hora_time AND ADDTIME(a.Hora_time,'00:14:59') 
        LEFT JOIN
