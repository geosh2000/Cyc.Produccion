  ...
  public function chgPDV_put(){
    ...
    $result = validateToken( $_GET['token'], $_GET['usn'], $func = function(){
      ...
      if(!$this->db->update( 'Asesores', $asesores )){
          $error['super']=$this->db->error();
          errResponse('Error en la base de datos de Aseosores', REST_Controller::HTTP_NOT_IMPLEMENTED, $this, 'error', $error);
          $flag = false;
      }
      ...
      // DepTable
      $this->db->query("SELECT depAsesores(".$data['asesor'].", ADDDATE(CURDATE(),365))");
