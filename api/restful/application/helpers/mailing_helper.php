...
class mailingV2{
    ...
    public static function sendMail($user, $type){
        ...
        $titulo = "Cambio de Rol ricardo.arballo (prueba)";
        $action = "solicitud de vacante";
        ...
        if( $type == 'plain' ){
            ...
            $msg= "Usuario Afectado:  ricardo.arballo\r\nPerfil Origen: Ventas In\r\nPerfil Nuevo: Ventas Out\r\nplain text";
        }else{
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $body= "<p>Usuario Afectado:  ricardo.arballo</p><p>Perfil Origen: Ventas In</p><p>Perfil Nuevo: Ventas Out</p><p>html</p>";
    ...
    public static function contratosVencidos( $class ){
        
        $mailQ = $class->db->query("SELECT 
        ...
        $mails = $mailQ->result_array();
        
        if( $mailQ->num_rows() == 0 ){
            return true;
        }
        
        $contQ = $class->db->query("SELECT 
        ...
        foreach($contratos as $index => $info){
            ...
            $body .= "<tr><td style='padding: 5px; border: 1px solid #d5d3d3;'>".$info['Nombre']."</td><td style='padding: 5px; border: 1px solid #d5d3d3;'>".$info['Dep']."</td><td style='padding: 5px; border: 1px solid #d5d3d3;'><span style='color: $color'>".$info['fin']."</span></td><td style='padding: 5px; border: 1px solid #d5d3d3;'><span style='color: $color'>".$info['st']."</span></td></tr>\n";
        }
        unset($index, $info);
        ...
        foreach( $mails as $index => $info ){
            $body = "<p>Hola ".$info['Nombre'].",</p><p>Estos son los contratos vencidos y próximos a vencer:</p>".$body;
            $msg = test::msg_encapsule($titulo, $body);
            
            if( test::send($info['usuario'], $titulo, $body) ){
                $class->db->query("INSERT INTO mail_dailyCheck VALUES (NULL, 'contratos', CURDATE(), '".$info['usuario']."', 1, NULL) ON DUPLICATE KEY UPDATE sent = 1");    
            }else{
                $class->db->query("INSERT INTO mail_dailyCheck VALUES (NULL, 'contratos', CURDATE(), '".$info['usuario']."', 0, NULL) ON DUPLICATE KEY UPDATE sent = 0");
            }
    ...
    public static function send($user, $title, $msg){

        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $headers .= "From: Notificaciones ComeyCome <operaciones@pricetravel.com>";

        if( mail("$user@pricetravel.com",$title,$msg,$headers) ){
            return true;
        }else{
            return false;
        }
    ...
    public static function msg_encapsule($title, $body){
        $msg= "<html xmlns=\"http://www.w3.org/1999/xhtml\" style=\"-webkit-box-sizing: border-box;box-sizing: border-box;font-family: sans-serif;line-height: 1.15;-ms-text-size-adjust: 100%;-webkit-text-size-adjust: 100%;-ms-overflow-style: scrollbar;-webkit-tap-highlight-color: transparent;\">\n";
        $msg.= "  <head style=\"-webkit-box-sizing: inherit;box-sizing: inherit;\">\n";
        $msg.= "    <meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" style=\"-webkit-box-sizing: inherit;box-sizing: inherit;\">\n";
        $msg.= "    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\" style=\"-webkit-box-sizing: inherit;box-sizing: inherit;\">\n";
