...
class DetalleAsesores extends REST_Controller {
  ...
  public function contrato_get(){
      ...
      $result = validateToken( $_GET['token'], $_GET['usn'], $func = function(){
          ...
          $query = "SELECT 
                        NOMBREASESOR(a.id,2) as nombre, Ingreso, Egreso, IF(Egreso<CURDATE(),0,1) AS activo, c.recontratable, d.tipo, inicio, fin, IF(fin IS NOT NULL AND fin<CURDATE() AND Egreso>CURDATE(), 1, 0) as vencido,
